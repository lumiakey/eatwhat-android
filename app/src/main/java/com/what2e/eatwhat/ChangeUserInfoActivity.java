...
public class ChangeUserInfoActivity extends BaseActivity implements View.OnClickListener {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_change_uesr_info);
        pg=new ProgressDialog(ChangeUserInfoActivity.this);
        getUserData();
        initView();
    }
    ...
    public void onClick(View v) {
        switch (v.getId()) {
            case R.id.btnChangeInfo:
                ...
                if (!Util.checkNetwork(ChangeUserInfoActivity.this)) {
                    pg.dismiss();
                    return;
                }
                new Thread() {
    ...
    Runnable runnableModfInfo = new Runnable() {
        ...
        public void run() {
            ...
            if (postForm == null || postForm.isEmpty()) {
                Util.showToast(ChangeUserInfoActivity.this, "服务器出错，请重试！");
                return;
            }
            if (postForm.length() > 100) {
                Util.showToast(ChangeUserInfoActivity.this, "内部服务器发生错误");
                return;
            } else {
                Gson gson = new Gson();
                ...
                if (statusCode == 200) {
                    sharedPreferences = PreferenceManager.getDefaultSharedPreferences(ChangeUserInfoActivity.this);
